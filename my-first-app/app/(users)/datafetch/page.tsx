...
const DataFetchComp = async ({ searchParams }: PropsPage) => {
  const userName = Array.isArray(searchParams?.name) ? searchParams.name[0] : searchParams?.name;
  ...
  return (
     <div className="grid grid-cols-2 gap-3 h-full ">
      ...
      <Suspense fallback={<Loading />}>
        <DataFetchServer user={{ name: userName }} />
      </Suspense>
    </div>
  )
}
