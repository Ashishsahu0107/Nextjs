...
export default function RegisterPage() {
  ...
  const getPasswordStrength = () => {
    ...
    switch (score) {
      case 1:
        return { score, label: 'Weak', color: 'bg-rose-500 w-1/4' };
      case 2:
        return { score, label: 'Fair', color: 'bg-amber-500 w-2/4' };
      case 3:
        return { score, label: 'Good', color: 'bg-indigo-500 w-3/4' };
      case 4:
        return { score, label: 'Strong', color: 'bg-emerald-500 w-full' };
      default:
        return { score: 0, label: 'Too Short', color: 'bg-rose-500 w-1/12' };
    }
  };
  ...
  return (
    <div className="pt-28 pb-20 min-h-screen text-slate-100 bg-slate-950 flex flex-col justify-center items-center relative overflow-hidden px-4">
      {/* Background Glows */}
      <div className="absolute top-[20%] right-[10%] w-[350px] h-[350px] rounded-full bg-cyan-500/10 blur-[100px] pointer-events-none -z-10" />
      <div className="absolute bottom-[20%] left-[10%] w-[350px] h-[350px] rounded-full bg-violet-600/10 blur-[100px] pointer-events-none -z-10" />
      ...
      <div className="w-full max-w-md glass p-8 sm:p-10 rounded-3xl border border-slate-900 shadow-2xl relative">
        ...
        <div className="text-center space-y-3 mb-8 relative z-10">
          ...
          <p className="text-xs text-slate-400">Start building your technical depth today</p>
        </div>
        ...
        {success ? (
          <div className="bg-emerald-950/60 border border-emerald-500/40 text-emerald-400 p-4 rounded-xl text-xs text-center space-y-3 mb-6 animate-fade-in relative z-10">
            ...
            <p className="text-[11px] text-emerald-500">Sending a verification email to your address...</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4 relative z-10 text-left">
            {/* Error message */}
            {error && (
              <div className="bg-rose-950/60 border border-rose-500/40 text-rose-400 p-3 rounded-xl text-xs font-semibold">
                ⚠ {error}
              </div>
            )}
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full px-4 py-2.5 bg-slate-950/80 border border-slate-900 hover:border-slate-800 focus:border-violet-500 focus:outline-none rounded-xl text-sm text-white placeholder-slate-650 transition-colors"
              />
            </div>
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full px-4 py-2.5 bg-slate-950/80 border border-slate-900 hover:border-slate-800 focus:border-violet-500 focus:outline-none rounded-xl text-sm text-white placeholder-slate-700 transition-colors"
              />
            ...
            <div className="flex items-start gap-2 pt-2">
              <input
                ...
                className="mt-0.5 w-4 h-4 rounded bg-slate-950 border-slate-900 text-violet-600 focus:ring-violet-500/20 focus:ring-offset-slate-950 focus:ring-2 cursor-pointer"
              />
              <label htmlFor="terms" className="text-[11px] text-slate-450 leading-tight select-none cursor-pointer">
                I agree to the{' '}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 bg-gradient-to-r from-violet-600 to-cyan-600 hover:from-violet-500 hover:to-cyan-500 text-sm font-semibold text-white rounded-xl shadow-[0_0_20px_rgba(139,92,246,0.2)] hover:shadow-[0_0_25px_rgba(139,92,246,0.35)] transition-all flex items-center justify-center gap-2 mt-4"
            >
              {loading ? (
                <>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6 relative z-10">
          {/* GitHub OAuth Button */}
          <button className="flex items-center justify-center gap-2 py-2 px-4 bg-slate-900/60 hover:bg-slate-900 border border-slate-800 hover:border-slate-700 text-xs font-semibold text-slate-200 rounded-xl transition-all">
